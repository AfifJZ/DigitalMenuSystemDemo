...
public class BranchManageController {
    ...
    @GetMapping("/manage/branch/{branchId}/password-plain")
    @Transactional(readOnly = true)
    public org.springframework.http.ResponseEntity<?> getBranchPassword(
            @PathVariable Long branchId,
            HttpSession httpSession
    ) {
        ManagerSession session = authService.getSession(httpSession);
        if (session == null || !session.isOrganizationLevel()) {
            return org.springframework.http.ResponseEntity.status(403).body("{\"error\":\"Unauthorized\"}");
        }
        Branch branch = loadOwnedBranch(branchId, session.getOrganizationId());
        if (branch == null) {
            return org.springframework.http.ResponseEntity.status(404).body("{\"error\":\"Branch not found\"}");
        }
        // Since passwords are hashed, we cannot retrieve them
        // Return a message indicating this
        return org.springframework.http.ResponseEntity.ok(new java.util.HashMap<String, String>() {{ 
            put("error", "Password cannot be displayed. It was securely hashed when saved.");
        }});
    }
